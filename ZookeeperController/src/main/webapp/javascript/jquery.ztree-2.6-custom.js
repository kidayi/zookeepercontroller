...
(function($) {
    ...
    function bindTreeNodes(setting, treeObj) {
        ...
        treeObj.unbind(ZTREE_ADD);
        treeObj.bind(ZTREE_ADD, function (event, treeId, treeNode) {
            tools.apply(setting.callback.add, [event, treeId, treeNode]);
        });
    ...
    function appendTreeNodes(setting, level, treeNodes, parentNode) {
        ...
        for (var i = 0, l = treeNodes.length; i < l; i++) {
            ...
            html.push("<a id='", node.tId, IDMark_A, "' treeNode", IDMark_A," onclick=\"", (node.click || ''),
                ...
                "' title='",node[setting.titleCol],"'>",node[setting.nameCol].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'),"</span></a><ul id='", node.tId, IDMark_Ul, "' class='", makeUlLineClass(setting, node), "' style='display:", (node.open ? "block": "none"),"'>");
    ...
    function addAddBtn(setting, treeNode){
        ...
        var aObj = $("#" + treeNode.tId + IDMark_A);
        var removeStr = "<button type='button' class='add' id='" + treeNode.tId + IDMark_Add + "' title='' treeNode"+IDMark_Add+" onfocus='this.blur();' style='display:none;'></button>";
        aObj.append(removeStr);

        $("#" + treeNode.tId + IDMark_Add).bind('click',
            function() {
                if (tools.apply(setting.callback.beforeAdd, [setting.treeObjId, treeNode], true) == false) return true;
                addTreeNode(setting, treeNode);
                //触发remove事件
                setting.treeObj.trigger(ZTREE_ADD, [setting.treeObjId, treeNode]);
                return false;
            }
        ).bind('mousedown',
    ...
    function addTreeNode(setting, treeNode){
        //to_do
    }
    //删除子节点
